<center>
<div style="width:7%">
<!--- ![](https://upload.wikimedia.org/wikipedia/commons/a/a8/SPBGU_logo.png) -->
![](https://rawgit.com/mburyakov/abelian-unipotent/slides/PITCHME/SPBGU_logo.png)
</div>
<div style="color:DarkSlateGray; font-size:0.7em; height:3em;">
Санкт-Петербургский Государственный Университет<br>
</div>

<div style="height:3em;">
<h3>
Линейные алгебраические группы параболического типа
</h3>
</div>

<div style="color:DarkSlateGray; font-size: 0.7em; height:5em;">
Буряков Михаил Александрович<br>
</div></center>


<div style="color:DarkSlateGray; right: 5%; font-size: 0.7em; text-align: right">
<br>
<p>
Научный руководитель:
<br>
д. ф.-м. н., профессор Вавилов Н. А.
</p>
<p>
Рецензент:
<br>
к. ф.-м. н., доцент Певзнер И. М.
</p>
</div>


---

## Линейные алгебраические группы параболического типа с абелевым унипотентным радикалом

- конструируются на основе рационального представления редуктивной группы
<br><br>
- обобщают:
  - параболические подгруппы групп Шевалле <br> (с абелевым унипотентным радикалом)
  - вырожденные ортогональные группы


Note:
Структурная теория линейных алгебраических групп изучается уже почти полтора века. Одной из центральных её тем является вопрос о нормальном строении групп точек алгебраических групп.
Наряду с задачей о нормальном строении, то есть задачей об отыскании всех нормальных подгрупп, также изучается и задача нахождения всех подгрупп, нормализуемых некоторой меньшей подгруппой.
В частности, это может быть элементарная подгруппа группы Шевалле, то есть подгруппа, порождённая всеми корневыми унипотентами.
Оба этих вопроса о нормальном строении хорошо изучены для групп Шевалле и для редуктивных групп. Однако для нередуктивных групп результатов известно достаточно мало.

В данной работе изучаются нередуктивные группы, а именно введено понятие **_линейных алгебраических групп параболического типа с абелевым унипотентным радикалом_**.

---

# Мотивация
<br>
## Внутренние модули Шевалле

- Если $P$ — параболическая подгруппа группы Шевалле,  
$P = L \rightthreetimes U$ — её разложение Леви, то на факторгруппах нижнего центрального ряда $U$ можно ввести структуру модуля.
- Если $U$ абелев, то структура модуля на всём $U$.

<br>

--------
<div class="footer">
Подгруппы максимальной параболической группы, нормализуемые элементарной подгруппой Леви, описаны в работе:
<br>
*Stavrova A. Alg. Colloq. 2009. Vol. 16, no. 04, –– P. 631–648*
</div>

Note:
Сейчас я попытаюсь объяснить, откуда возникла идея ввести понятие алгебраических групп параболического типа с абелевым унипотентным радикалом.

Рассмотрим параболическую подгруппу группы Шевалле. Как известно, она раскладывается в полупрямое произведение
редуктивной части и унипотентного радикала. Это разложение называют разложением Леви.
При этом на факторгруппах нижнего центрального ряда унипотентного радикала можно ввести структуру модуля.

Если унипотентный радикал абелев, а именно этот случай и будет обобщаться, то он является векторной группой, то есть аддитивной группой внутреннего модуля Шевалле.
---

# Мотивация

## Вырожденные ортогональные группы

- Группа матриц, сохраняющих вырожденную билинейную форму вида
<center><div style="width:30%">
<!--- ![](https://tex.s2cms.ru/svg/%0A%5Ctextrm%7B%5Chuge%20%24B%24%7D%3D%5Cbegin%7Bpmatrix%7D%0A%20%20%20%20%26%20%20%26%20%20%20%20%26%20%20%5Cmulticolumn%7B1%7D%7B%7Cc%7D%7B%7D%20%26%20%20%20%20%20%20%20%20%20%20%20%5C%5C%0A%20%20%20%20%26%20%5Ctextrm%7B%5Chuge%20%24B_%7B%5Ctext%7B%D0%BD%D0%B5%D0%B2%D1%8B%D1%80%7D.%7D%24%7D%20%26%20%20%20%20%26%20%20%5Cmulticolumn%7B1%7D%7B%7Cc%7D%7B%7D%20%26%20%5Ctextrm%7B%5Chuge%200%7D%20%5C%5C%0A%20%20%20%20%26%20%20%26%20%20%20%20%26%20%20%5Cmulticolumn%7B1%7D%7B%7Cc%7D%7B%7D%20%26%20%20%20%20%20%20%20%20%26%20%20%20%20%5C%5C%0A%5Cmulticolumn%7B3%7D%7Bc%7D%7B%24%5Cupbracefill%24%7D%26%5Cmulticolumn%7B1%7D%7B%7Cc%7D%7B%7D%5C%5C%0A%5Cmulticolumn%7B3%7D%7Bc%7D%7B%5Cscriptstyle%20m%7D%26%5Cmulticolumn%7B1%7D%7B%7Cc%7D%7B%7D%5C%5C%0A%5Ccline%7B2-5%7D%0A%20%20%20%20%20%20%20%26%20%20%20%20%20%20%20%20%26%20%20%20%20%20%26%20%20%5Cmulticolumn%7B1%7D%7B%7Cc%7D%7B%7D%20%26%20%20%20%20%20%20%20%20%5C%5C%0A%20%20%20%20%20%20%20%26%20%20%20%5Ctextrm%7B%5Chuge%200%7D%20%20%20%20%20%26%20%20%20%20%20%26%20%20%5Cmulticolumn%7B1%7D%7B%7Cc%7D%7B%7D%20%26%20%5Ctextrm%7B%5Chuge%200%7D%20%5C%5C%0A%5Cmulticolumn%7B6%7D%7Bc%7D%7B%24%5Cupbracefill%24%7D%5C%5C%0A%5Cmulticolumn%7B6%7D%7Bc%7D%7B%5Cscriptstyle%20n%7D%5C%5C%0A%5Cnoalign%7B%5Cvspace%7B-2%5Cnormalbaselineskip%7D%7D%0A%5Cend%7Bpmatrix%7D%20%0A) -->
![](https://rawgit.com/mburyakov/abelian-unipotent/slides/PITCHME/formula1.svg)
</div></center>
будет состоять из матриц вида
<center><div style="width:30%">
<!--- ![](https://tex.s2cms.ru/svg/%0A%5Cbegin%7Bpmatrix%7D%0A%20%20%20%20%26%20%20%26%20%20%26%20%20%20%20%5Cmulticolumn%7B1%7D%7B%7Cc%7D%7B%5Cphantom%7B*%7D%7D%20%20%20%20%26%20%7B*%7D%20%26%20%7B*%7D%20%20%20%20%20%20%20%20%20%20%20%5C%5C%0A%20%20%20%20%26%20%5Ctextrm%7B%5Chuge%20%24O%28B_%7B%5Ctext%7B%D0%BD%D0%B5%D0%B2%D1%8B%D1%80%7D%2E%7D%29%24%7D%20%26%20%20%20%26%20%5Cmulticolumn%7B1%7D%7B%7Cc%7D%7B%7D%20%20%26%20%7B*%7D%20%26%20%7B*%7D%20%20%5C%5C%0A%20%20%20%20%26%20%20%26%20%20%20%20%26%20%5Cmulticolumn%7B1%7D%7B%7Cc%7D%7B%7D%20%20%26%20%20%20%20%20%7B*%7D%20%26%20%7B*%7D%20%20%26%20%20%20%20%5C%5C%0A%5Cmulticolumn%7B3%7D%7Bc%7D%7B%24%5Cupbracefill%24%7D%26%5Cmulticolumn%7B1%7D%7B%7Cc%7D%7B%7D%20%26%20%7B*%7D%20%26%20%7B*%7D%5C%5C%0A%5Cmulticolumn%7B3%7D%7Bc%7D%7B%5Cscriptstyle%20m%7D%26%5Cmulticolumn%7B1%7D%7B%7Cc%7D%7B%7D%5C%5C%0A%5Ccline%7B2-6%7D%0A%20%20%20%20%20%20%20%26%20%20%20%20%20%20%20%20%26%20%20%20%20%20%26%20%5Cmulticolumn%7B1%7D%7B%7Cc%7D%7B%7D%20%20%26%20%20%20%20%20%20%20%20%5C%5C%0A%20%20%20%20%20%20%20%26%20%20%20%5Ctextrm%7B%5Chuge%200%7D%20%20%20%20%20%20%20%26%20%26%20%5Cmulticolumn%7B3%7D%7B%7Cc%7D%7B%5Ctextrm%7B%5Chuge%20%24GL_%7Bn-m%7D%24%7D%20%26%20%20%20%20%5C%5C%0A%5Cmulticolumn%7B6%7D%7Bc%7D%7B%24%5Cupbracefill%24%7D%5C%5C%0A%5Cmulticolumn%7B6%7D%7Bc%7D%7B%5Cscriptstyle%20n%7D%5C%5C%0A%5Cnoalign%7B%5Cvspace%7B-2%5Cnormalbaselineskip%7D%7D%0A%5Cend%7Bpmatrix%7D%20%0A) -->
![](https://rawgit.com/mburyakov/abelian-unipotent/slides/PITCHME/formula2.svg)
</div></center>

---

# Определение

- Пусть $G\_\mathbb{Z}$ — связная расщепимая редуктивная групповая схема с системой корней $\Phi = \Phi\_1 + \dots + \Phi\_n$
- $\Phi\_i$ — приведённые неприводимые системы корней
- $\pi:G\_\mathbb{Z} \to \mathrm{GL} (U\_\mathbb{Z}) $ — рациональное представление $G\_\mathbb{Z}$ над $\mathbb{Z}$, становящееся неприводимым над алгебраически замкнутым полем характеристики $0$
- $U:=U\_\mathbb{Z}\otimes R$, $G := G\_\mathbb{Z}(R)$, $R$ — коммутативное кольцо

Тогда можно рассмотреть группу $P:=G\rightthreetimes U$, являющуюся полупрямым произведением $G$ и аддитивной группы модуля $U$,
c элементами вида $(g,u)$, $g \in G$, $u \in U$ и умножением:
$$
(g,u)\cdot (g',u') = (g g', {}^{g'^{-1}} u + u')
$$

Назовём такую группу **_группой параболического типа с абелевым унипотентным радикалом_**.

Note:
Таким образом, оказывается логичным следующее определение:

Если есть редуктивная групповая схема G\_Z,  
pi — её рациональное представление,  
G — её группа точек над комм. кольцом R,

то если рассматривать модуль U как векторную группу, можно сконструировать
полупрямое произведение группы G и векторной группы U.

Элементами этого полупрямого произведения будут пары с естественным умножением.

Такую группу будем называть группой параболического типа с абелевым унипотентным радикалом.

---

# Веса представления
$U$ — свободный модуль, раскладывающийся в прямую сумму<br>
весовых подпространств
<center>
<!--- <img class="upmath" src="https://tex.s2cms.ru/svg/U%3D%5Cbigoplus_%7B%5Csigma%20%5Cin%20%5CSigma%20%5Csubseteq%20X(T)%7D%20%7BU_%5Csigma%7D" alt="U=\bigoplus_{\sigma \in \Sigma \subseteq X(T)} {U_\sigma}" /> -->
<img class="upmath" src="https://rawgit.com/mburyakov/abelian-unipotent/slides/PITCHME/formula3.svg" alt="U\;=\;\bigoplus_{\sigma \in \Sigma}\;{U_\sigma}" />
</center>


Cтарший вес представления обозначим $\widetilde \sigma$

---------

## Элементарная группа $\;E \le G$

$$ E = \langle x\_\alpha(\xi) \;\|\; \alpha \in \Phi, \xi \in R \rangle $$

---------

<center>
<!--- <img class="upmath" src="https://tex.s2cms.ru/svg/%5Cmathrm%7Bln%7D%5C%3Bx_%5Calpha(%5Cxi)%20%3D%20(x_%5Calpha(%5Cxi)-1)%20-%20%5Cfrac%7B(x_%5Calpha(%5Cxi)-1)%5E2%7D%7B2%7D%20%2B%20%5Cfrac%7B(x_%5Calpha(%5Cxi)-1)%5E3%7D%7B3%7D%20-%20%5Cldots" alt="\mathrm{ln}\;x_\alpha(\xi) = (x_\alpha(\xi)-1) - \frac{(x_\alpha(\xi)-1)^2}{2} + \frac{(x_\alpha(\xi)-1)^3}{3} - \ldots" /> -->
<img class="upmath" src="https://rawgit.com/mburyakov/abelian-unipotent/slides/PITCHME/formula4.svg" alt="\mathrm{ln}\;x_\alpha(\xi) = (x_\alpha(\xi)-1) - \frac{(x_\alpha(\xi)-1)^2}{2} + \frac{(x_\alpha(\xi)-1)^3}{3} - \ldots" />
</center>

Note:
В работе используются основные понятия теории представлений.

В частности, рациональные представления представляют собой модули старшего веса.

Корневые унипотенты, породжающие элементарную группу, действуют как операторы на модуле U и, предположив обратимость
 в кольце R некоторых целых чисел,
можно взять логарифм этих операторов, который переводит одни весовые подпространства в другие.

---

# Теорема $2$

При выполнении некоторых условий в группе $P:=G\rightthreetimes U$ <br>
параболического типа с абелевым унипотентным радикалом подгруппы, нормализуемые элементарной подгруппой $E\le G$, <br>
имеют вид
<center>
$H = H\_G \rightthreetimes H\_U$,
</center>

где
<br>
<b style="word-space:10em">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</b>
$[H\_G , E]$ = $\prod\_{i=1}^n E(Φ\_i , R, I\_i )$,<br>
<b style="word-space:10em">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</b>
$H\_U = U\_I$ — подмодуль $U$, заданный некоторым идеалом кольца.

Note:
Основным результатом работы является теорема, описывающая подгруппы, нормализуемые элементарной.

При соблюдении некоторых условий, которые мы обсудим чуть позже, подгруппы, нормализуемые элементарной,
распадаются в полупрямое произведение редуктивной и унипотентной части.

---

## Условия:

1. $rk \; Φ_i \ge 2$
<br><br>
1. $2 \in R^* $, если среди $Φ_i$ есть $B_l$, $C_l$ или $F_4$
1. $3 \in R^* $, если среди $Φ_i$ есть $G_2$
1. $2 \ldots n \in R^* $, так что определены $\mathrm{ln}\;x_\alpha(\xi)$
<br><br>
1. ${\vphantom{\bigg\langle}}^E \bigg\langle {}^{EB}\langle u \rangle \cap U\_{\widetilde\sigma} \bigg\rangle = {\vphantom{\big\langle}}^E \big\langle u \big\rangle$
для любого весового вектора $u$.
<br><br>
1. Ни для какой неприводимой компоненты $Φ\_i$<br>
с максимальным весом $\widetilde{β}\_i$<br>
разность $\widetilde{σ} - \widetilde{β}\_i$  не оказывается ортогональна всей $Φ\_i$.

---

## Условия:

- ${\vphantom{\bigg\langle}}^E \bigg\langle {}^{EB}\langle u \rangle \cap U\_{\widetilde\sigma} \bigg\rangle = {\vphantom{\big\langle}}^E \big\langle u \big\rangle$
для любого весового вектора $u$.

----------------

Если последовательно коммутировать $u$ с положительными корневыми элементами, в какой-то момент получится некоторый вектор старшего веса.
<br>
Полученные таким образом вектора старшего веса должны в совокупности порождать тот же $E$-инвариантный подмодуль, что и исходный вектор $u$.

----------------

Следствие:

Решётка $E$-инвариантных подмодулей $U$ определяется решёткой идеалов $R$.


Note:
Принципиально это условие означает обратимость некоторых констант представления, то есть его нужно рассматривать, как условие на кольцо.

Для каждого конкретного представления это условие легко проверяется, то есть можно понять, какие кольца удовлетворяют этому условию при фиксированном представлении.

Например, присоединённое представление $A\_2$ и произвольный вектор нулевого веса.

---

## Условия:

- Ни для какой неприводимой компоненты $Φ\_i$ с максимальным корнем $\widetilde{β}\_i$ разность $\widetilde{σ} - \widetilde{β}\_i$  не ортогональна всей $Φ\_i$.

<center>
<div>
<img width="60%" src="https://rawgit.com/mburyakov/abelian-unipotent/slides/PITCHME/formula5.svg" alt="
\tdplotsetmaincoords{75}{70}
\begin{tikzpicture}[scale=1.5,tdplot_main_coords]
\foreach\ang in {0,60,120}{
  \draw[ultra thick,blue,-{>[length=10,width=5]}] (0,0) -- ++(\ang+30:3);
  \draw[ultra thick,olive,-{>[length=10,width=5]}] (0,0) -- ++(\ang+180+30:3);
}
\path (0,0) ++(30:3) node [label=below left:{\huge $\alpha_1$}] {};
\path (0,0) ++(150:3) node [label=right:{\huge $\alpha_2$}] {};
\path (0,0) ++(90:3) node [label=right:{\huge $\widetilde\beta$}] {};
\draw[ultra thick,red,dashed] (0,3,-3) -- (0,3,2.7) {};
\draw[very thick,dotted] (0,-3,-2) -- (0,-3,2) {};
\draw[very thick,dotted] (2.5980,1.5,-2) -- (2.5980,1.5,2) {};
\draw[very thick,dotted] (-2.5980,-1.5,-2) -- (-2.5980,-1.5,2) {};
\draw[very thick,dotted] (-2.5980,1.5,-2) -- (-2.5980,1.5,2) {};
\draw[very thick,dotted] (2.5980,-1.5,-2) -- (2.5980,-1.5,2) {};
\draw[very thick,dotted] (2.5980,-1.5,2) -- (2.5980,1.5,2) {};
\draw[very thick,dotted] (2.5980,1.5,2) -- (0,3,2) {};
\draw[very thick,dotted] (0,3,2) -- (-2.5980,1.5,2) {};
\draw[very thick,dotted] (-2.5980,1.5,2) -- (-2.5980,-1.5,2) {};
\draw[very thick,dotted] (-2.5980,-1.5,2) -- (0,-3,2) {};
\draw[very thick,dotted] (0,-3,2) -- (2.5980,-1.5,2) {};
\draw[very thick,dotted] (2.5980,-1.5,-2) -- (2.5980,1.5,-2) {};
\draw[very thick,dotted] (2.5980,1.5,-2) -- (0,3,-2) {};
\draw[very thick,dotted] (0,3,-2) -- (-2.5980,1.5,-2) {};
\draw[very thick,dotted] (-2.5980,1.5,-2) -- (-2.5980,-1.5,-2) {};
\draw[very thick,dotted] (-2.5980,-1.5,-2) -- (0,-3,-2) {};
\draw[very thick,dotted] (0,-3,-2) -- (2.5980,-1.5,-2) {};
\node[very thick,blue] at (120:3) {\huge $\Phi^+$};
\node[very thick,olive] at (180+120:2.5) {\huge $\Phi^-$};
\end{tikzpicture}
">
</div>
</center>

Note:
И последнее, самое существенное условие  
...
Я попытался изобразить это на примере неприводимой компоненты типа A2 -  
здесь старший вес не должен лежать на красной пунктирной прямой.

Это условие проходит через всё доказательство.  
Доказательство построено на вычислениях в решётке весов,  
а это условие - на вид многогранника весов представления.  
Поэтому это условие существенно используется в доказательстве.

А если это условие не выполняется, то есть старший вес лежит в 
этом исключительном многообразии, то такая ситуация является существенным
исключением.  

---

## Случай присоединённого представления

<br>

Теорема $3$.

<br>
Пусть выполняются условия теоремы $2$  
за исключением требования $\widetilde{σ} - \widetilde{β}\_i \not\perp Φ\_i \; ∀i$.  
А именно, пусть $\widetilde{σ} - \widetilde{β}\_i \perp Φ\_i$ для некоторого $i$.

Тогда если $x\_{β\_i}(ξ) ∈ H\,U$, то
$$x\_{β\_i}(ξ^2) ∈ H.$$

Note:
Наиболее наглядным и по существу единственным примером такой ситуации является присоединённое
представление и представления, строящиеся на его основе.  
Для таких представлений утверждение теоремы не выполняется, а вместо этого
верно более слабое условие, приводящее к более сложно устроенной решётке подгрупп,
нормализуемых элементарной.

---

# Обобщение

Параболические подгруппы с унипотентным радикалом класса нильпотентности $2$ являются частным случаем следующей конструкции:

- $U$ — $G$-модуль
- $\circledast : U \otimes U \to U$
- $ {}^g u \circledast {}^g v = {}^g (u \circledast v)$
- $V := \mathrm{Im}({\circledast})$
- $U = U\\\!/V \oplus V$
- $U \circledast V = V \circledast U = 0$

Тогда можно рассмотреть группу $P:=G\rightthreetimes U$, являющуюся расширением $G$ с помощью $U$, элементами которой будут пары $(g,u)$, $g \in G$, $u \in U$ со следующим умножением:
$$
(g,u)\cdot (g',u') = (g g', {}^{g'^{-1}} u + u' + {}^{g'^{-1}} u \circledast u')
$$

---

# Выводы

- Получено доказательство теоремы о нормальном строении линейных алгебраических групп параболического типа с абелевым унипотентным радикалом.
- Установлено, что при выполнении некоторых условий любая подгруппа, нормализуемая элементарной, распадается в полупрямое произведение редуктивной и унипотентной частей.
- В исключительном случае, когда $\widetilde\sigma - \widetilde\beta\_i \perp \Phi\_i$, верно более слабое условие на подгруппы, нормализуемые элементарной.
- Сформулировано обобщающее понятие линейных алгебраических групп параболического типа с унипотентным радикалом класса нильпотентности 2.
- Описание нормального строения для аналогичных групп с унипотентным радикалом более высоких классов нильпотентности является предметом дальнейших исследований.
