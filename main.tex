...
\newenvironment{framed}
    {
\vspace{1.5ex}
\begin{tcolorbox}[colback=white, grow to left by=1.2em, grow to right by=1.2em, breakable, enhanced]
    }
    {
\end{tcolorbox}
    }
...
Работа устроена следующим образом: во втором \# разделе представлено решение вопроса о нормальном строении параболически-сходных групп с абелевым унипотентным радикалом. В третьем разделе обсуждается существенное условие, возникающее в доказательствах второй части, и его влияние на расположение подгрупп, нормализуемых элементарной.
...
\begin{equation}\label{representationrank}
\mathrm{rk} \; U_\Z \ge 2 \;.
\end{equation}
...
Над кольцами, для которых выполняется (\ref{returnfromhighest}), инвариантные подмодули $U$ имеют вид $U_I = U \cdot I$, где $I \trianglelefteq R$ --- некоторый идеал.
\end{lm}
\begin{proof}
...
а значит по условию предыдущей леммы $\langle\sigma,\delta-\gamma\rangle > 0$, то есть выполнено условие (0).
\end{proof}
...
\end{thm}
\begin{proof}
Достаточно доказать, что $H_G \le H$, оставшаяся часть следует из \ref{unipotenttransitivity}.
